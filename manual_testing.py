...
# pl.get_data()
pl.insert_entry(['jmlindict2'], {'foo': ['foo'], 'bar': ['foo', 'bar'], 'baz': ['foo', 'bar', 'baz']})
pl.get_data()
pl.insert_entry(['jmlindict3'], {'foo': ['foo', 0, 0], 'bar': ['foo', 'bar', 1], 'baz': ['foo', 'bar', 'baz', open]})
pl.get_data()
...
pl = PlistHelper(b'<plist><string></string></plist>')
pl.merge(b'<plist><dict><key>foo</key><string>bar</string></dict></plist>', overwrite=True)
pl.print()

pl = PlistHelper(b'<plist><string></string></plist>')
pl.merge(b'<plist><dict><key>one</key><integer>1</integer></dict></plist>', overwrite=True)
pl.print()
...
pl = PlistHelper(b'<plist><string></string></plist>')
pl.merge(b'<plist><array><string>one</string><string>two</string><string>three</string></array></plist>', overwrite=True)
pl.print()

pl = PlistHelper(b'<plist><array><string>one</string><string>two</string><string>three</string></array></plist>')
pl.merge(b'<plist><array><string>cero</string><string>one</string><string>dos</string><string>three</string><string>quatro</string></array></plist>', overwrite=True)
pl.print()
